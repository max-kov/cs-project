...
class GameState:
    ...
    def set_pool_balls(self):
        ...
        coord_shift = np.array(
            [math.sin(math.radians(60)) * config.ball_radius *
    ...
    def check_remaining(self):
        ...
        ball_type_remaining = {BallType.Solid: solids_remaining,
                               BallType.Striped: stripes_remaining}

        # decides if on of the players (or both) should be potting 8ball
        self.potting_8ball = {Player.Player1: not ball_type_remaining[
    ...
    def first_collision(self, ball_combination):
        ...
        self.white_ball_1st_hit_8ball = ball_combination[0].number == 8 or \
                                        ball_combination[1].number == 8
        ...
        if len(self.potted) > 0:
            ...
            if only_solids_potted or only_stripes_potted:
                ...
                if self.ball_assignment is None:
                    ...
                    self.potting_8ball = {self.current_player: False,
                                          other_player: False}
                elif self.ball_assignment[
                    self.current_player] != selected_ball_type:
    ...
    def first_hit_rule(self):
        ...
        if not self.white_ball_1st_hit_is_set:
            ...
            if not self.white_ball_1st_hit_8ball and self.ball_assignment[
                ...
                self.turn_over(True)
            # checks if the 8ball was the first ball hit, and
            # if so checks if the player needs to pot the 8ball

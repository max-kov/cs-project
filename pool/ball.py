        ...
        for num, point in enumerate(self.stripe_circle[:-1]):
            if point[2] >= -1:
                pygame.draw.line(sprite, (255, 255, 255),
                                 ...
                                 config.ball_radius + self.stripe_circle[
                                                          num + 1][:2],
...
class BallSprite(pygame.sprite.Sprite):
    ...
    def update(self, *args):
        if np.hypot(*self.ball.velocity) != 0:
            ...
            rotation_angle = -np.hypot(*(self.ball.velocity)) / config.ball_radius
    ...
    def update_sprite(self):
        ...
        if self.label_offset[2] > 0:
            ...
            if self.number != 0:
                label.blit(self.text,
                           (config.ball_radius - self.text_length) / 2)
    ...
    def is_clicked(self, events):
        return physics.distance_less_equal(events["mouse_pos"], self.ball.pos,
                                           config.ball_radius)
        ...
        while events["clicked"]:
            ...
            if np.all(np.less(
                ...
                if behind_separation_line:
                    if events["mouse_pos"][0] <= config.white_ball_initial_pos[
                        0]:

...
def add_separation_line(canvas):
    ...
    pygame.draw.line(canvas.background, config.separation_line_color,
                     (config.white_ball_initial_pos[0], 0),
...
def create_buttons(text, text_font, text_color_normal, text_color_on_hover):
    # this function generates button objects using button text, button font,
    # normal colour and color of the text when
    # the mouse is hovering over the button

    button_size = np.array(
        [text_font[num].size(text[num]) for num in range(len(text))])
    ...
    screen_button_middles = np.stack((np.repeat([screen_mid], len(buttons)),
                                      np.arange(len(buttons)) * change_in_y),
                                     axis=1)
...
def draw_rects(button_size, buttons, game_state, text_starting_place,
               emit=list()):

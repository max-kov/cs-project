...
This project is about making a simple 8 ball pool game with a top down view. The game will:

* be played by 2 people on a single computer
* have use the standard pool rule set
* have realistic ball animations and behaviour (such as realistic collisions and friction)
...
According to the Tiobe Index (https://www.tiobe.com/tiobe-index/) and PYPL (http://pypl.github.io/PYPL.html), python is significantly more popular than Pascal and hence is a better language to use due to more resources available online (e.g. more StackOverlow questions/answers). This also suggests that python will have higher quality/variety of libraries due to a larger community. Additionally, python is a duck-typed language, which means no explicit type declarations are required. This makes python less verbose and more flexible, allowing for faster development, which is beneficial given a limited amount of time for development. Finally, a game development library will be at the core of my project so it is important to pick a language with a mature, well maintained game development framework.
...
The game will be written in Python and some additional libraries:
...
Python has an in-build library called turtle[@pythonturtle], however it’s mainly for education purposes and does not nearly have the functionality for making a pool game. I will be using Pygame for this projects, because of aforementioned reasons.

#### Numpy

Numpy[@numpy] is a powerful tool for numerical calculations often involving matrices. In this project matrices are used for drawing the ball sprites, specifically 3d rotation matrices.

#### Zope.event
This[@zope] is a very simple event handling/posting module.It will be used to handle in-game events, like potted balls or collisions.

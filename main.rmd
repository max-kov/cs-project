...
The main body of the ball, is a big circle we put everything else on top. A pure sphere of a solid color will look like a circle from any angle in a 2-dimensions. That's why we don't need to worry about any transformations here.

The line on the side of the ball is rendered by setting many 3-dimensional around the balls circumference when it is initialised and then translated using rotation matrices. The 3d matrix is calculated from an axis and an angle. We know the axis of rotation - it is the line perpendicular to the velocity vector and the angle of rotation is calculated using the formula
$$
\theta=\frac{vt}{r}
$$
...
Every frame the rotation matrix is calculated and applied to the line. When the line is drawn, if the z-axis value of a point is less than $0$, the point is ignored. That is because that point is *under* the ball, and cannot be seen from a top-down view. Other points are traced with a line.

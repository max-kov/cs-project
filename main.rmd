...
The main body of the ball, is a big circle we put everything else on top. A pure sphere of a solid color will look like a circle from any angle in a 2-dimensional system. That's why we don't need to worry about any transformations here.

The line on the side ball is rendered by setting many 3-dimensional around the balls circumference when it is initialised and then using rotation matrices to transform the points. The 3d matrix is calculated using a line-angle formula. We know the line around which the ball will be rotating - it is the line perpendicular to the velocity vector and the angle of rotation is calculated using the formula
$$
\theta=\frac{vt}{r}
$$
...
The main body will be tracked with a single point. That point will be in the centre of the ball when it's initialised and will be translated with the same rotation matrix outlined earlier. Additionally, the label will be compressed or *squashed* depending on its position on the circle. For example, when the label is on the circumference of the ball, the width of the label sprite is squashed to values close to 0. To get the scale factor of this transformation, I will use the formula $SF=rd$, where $r$ is the radius of the ball, and $d$ is the distance of the label to the edge of the ball from the top-down view. One important thing to note is that this is a __heuristic__ method. This technique produces believable results, but isn't how it exactly works in real life. One method which would produce better results is __surfel__ modeling[@surfel], but implementing something that complex is out of the scope of this project.
...
![table_sprites](figures/object_diagram_tablesprites.png)
